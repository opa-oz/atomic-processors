...
namespace AtomicProcessors::CSV2JSON {
    ...
    void init(CLI::App &app) {
        ...
        csv2json->callback([&]() { process(_input_path, _title_mapping_json, _delimiter, _quotechar, _output_path); });
        ...
        csv2json_f->add_option("--input2", _title_mapping_path, "File with header mapping object.")->required()->check(
                CLI::ExistingFile);
        ...
        csv2json_f->callback([&]() {
            CSV2JSON::process(_input_path, Utils::Read::file2string(_title_mapping_path), _delimiter, _quotechar,
                              _output_path);
        });
    }
}
